...
void filter(char *source, char *search){
...
void print_tcp_stat(Ipv4InfoList *tcp_list, Ipv6InfoList *tcp6_list){
...
void print_udp_stat(Ipv4InfoList *udp_list, Ipv6InfoList *udp6_list){
			...
			if(fddir){
				while((fdfile = readdir(fddir)) != NULL){
					...
					if (inode == ino){
						...
						*/
						sprintf(pid_buffer, "%s/%s", pid, pname);
...
int main(int argc, char *argv[]) {
	...
	if ((print_mode & 1) == 1){
		...
		print_tcp_stat(tcp_info_list, tcp6_info_list);
	}
	if ((print_mode & 2) == 2){
		udp_info_list = get_ipv4_info(udp_path);
                udp6_info_list = get_ipv6_info(udp6_path);
		print_udp_stat(udp_info_list, udp6_info_list);
	}
